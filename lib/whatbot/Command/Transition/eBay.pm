...
sub do_query {
    ...
    my $result = eval { XMLin($content,
        #KeyAttr => {NameValueList => 'Name'},
        ForceArray => [ 'NameValueList' ],
        GroupTags => {ItemSpecifics => 'NameValueList'}
    ); };
...
sub _hash_to_price {
    ...
    return undef unless (defined($pricehash) && ref($pricehash) eq "HASH");
