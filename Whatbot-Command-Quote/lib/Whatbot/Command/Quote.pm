...
class Whatbot::Command::Quote extends Whatbot::Command
	...
	method register(...) {
		...
		$self->_init_random_quote( $self->my_config );
	...
	method add_quote( $message?, $captures? ) : GlobalRegEx('^quote (.*?)[,\:]? "(.*?)"\s*$') {
	...
	method get_quote_by_id( $message?, $captures? ) : GlobalRegEx('^quote (\d+)$') {
	...
	method _calculate_random_time() {
		return ( $self->random_minimum + int( rand( $self->random_maximum - $self->random_minimum ) ) );
	}

	method _init_random_quote($config) {
		if ( $config and my $rq = $config->{random_quote} ) {
			my $start = $rq->{start};
			my $end = $rq->{end};
			my $days = $rq->{days};
